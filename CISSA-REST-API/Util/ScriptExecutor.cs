...
namespace ASIST_REPORT_REST_API.Util
{
    public static class ScriptExecutor
    {
        ...
        {
            public static readonly Guid BankAccountDefId = new Guid("{BE6D5C1F-48A6-483B-980A-14CEFF781FD4}");//Банковский счет
            public static readonly Guid AppDefId = new Guid("{4F9F2AE2-7180-4850-A3F4-5FB47313BCC0}"); //Заявления
            public static readonly Guid PersonDefId = new Guid("{6052978A-1ECB-4F96-A16B-93548936AFC0}");
            public static readonly Guid AppStateDefId = new Guid("{547BBA55-2281-4388-A1FC-EE890168AC2D}"); //Анкета домохозяйства
            ...
            public static readonly Guid PaymentDefId = new Guid("{68667FBB-C149-4FB3-93AD-1BBCE3936B6E}"); //Оплата
            public static readonly Guid AssignmentDefId = new Guid("{51935CC6-CC48-4DAC-8853-DA8F57C057E8}");
            
            public static readonly Guid onPaymentStateId = new Guid("{78C294B5-B6EA-4075-9EEF-52073A6A2511}");

            public static GeneralReportItem Execute(Guid districtId, Guid?djamoatId)
            {
                var context = CreateAsistContext("sod_user", new Guid("{4296EF4D-ED7A-41F8-BE88-2684AD21AC0E}"));
                var docRepo = context.Documents;
                var distrId = (Guid?)districtId ?? Guid.Empty;
                var djamId = (Guid?)djamoatId ?? Guid.Empty;
                return Build(context, distrId, djamId);
            }

            public static GeneralReportItem Build(WorkflowContext context, Guid distrId, Guid djamId)
            {
                var docRepo = context.Documents;
                var generalReportItem = new GeneralReportItem();
                if (djamId != Guid.Empty)
                ...
                using (var query = context.CreateSqlQuery(qb.Def))
                {
                    ...
                    var personSrc = query.JoinSource(appSrc, PersonDefId, SqlSourceJoinType.Inner, "Person");
                    var appStateSrc = query.JoinSource(appSrc, AppStateDefId, SqlSourceJoinType.Inner, "Application_State");
                    var distrSrc = query.JoinSource(appStateSrc, DistrictDefId, SqlSourceJoinType.Inner, "DistrictId");
                    var djamoatSrc = query.JoinSource(appStateSrc, DjamoatDefId, SqlSourceJoinType.LeftOuter, "DjamoatId");
                    var villageSrc = query.JoinSource(appStateSrc, VillageDefId, SqlSourceJoinType.LeftOuter, "VillageId");
                    var paymentSrc = query.JoinSource(query.Source, PaymentDefId, SqlSourceJoinType.LeftOuter, "BankAccount");
                    var assignSrc = query.JoinSource(paymentSrc, AssignmentDefId, SqlSourceJoinType.LeftOuter, "Assignment");

                    query.AndCondition(appSrc, "&State", ConditionOperation.Equal, onPaymentStateId);
                    ...
                    var gr1 = query.AddAttribute(query.Source, "&Id");
                    var gr2 = query.AddAttribute(query.Source, "Account_No");
                    var gr3 = query.AddAttribute(appSrc, "No");
                    var gr4 = query.AddAttribute(personSrc, "Last_Name");
                    var gr5 = query.AddAttribute(personSrc, "First_Name");
                    var gr6 = query.AddAttribute(personSrc, "Middle_Name");
                    var gr7 = query.AddAttribute(personSrc, "PassportSeries");
                    var gr8 = query.AddAttribute(personSrc, "PassportNo");
                    var gr9 = query.AddAttribute(distrSrc, "Name");
                    var gr10 = query.AddAttribute(djamoatSrc, "Name");
                    var gr11 = query.AddAttribute(villageSrc, "Name");
                    var gr12 = query.AddAttribute(appStateSrc, "street");
                    var gr13 = query.AddAttribute(appStateSrc, "House");
                    var gr14 = query.AddAttribute(appStateSrc, "flat");
                    var gr15 = query.AddAttribute(appSrc, "Date");
                    query.AddAttribute(assignSrc, "Amount", SqlQuerySummaryFunction.Sum);

                    query.AddGroupAttribute(gr1);
                    query.AddGroupAttribute(gr2);
                    query.AddGroupAttribute(gr3);
                    query.AddGroupAttribute(gr4);
                    query.AddGroupAttribute(gr5);
                    query.AddGroupAttribute(gr6);
                    query.AddGroupAttribute(gr7);
                    query.AddGroupAttribute(gr8);
                    query.AddGroupAttribute(gr9);
                    query.AddGroupAttribute(gr10);
                    query.AddGroupAttribute(gr11);
                    query.AddGroupAttribute(gr12);
                    query.AddGroupAttribute(gr13);
                    query.AddGroupAttribute(gr14);
                    query.AddGroupAttribute(gr15);                    
                    ...
                    {
                        var bankAccountId = row[0] is DBNull ? Guid.Empty : (Guid)row[0];
                        var accNo = row[1] is DBNull ? "-" : (string)row[1];
                        var appNo = row[2] is DBNull ? "-" : (string)row[2];
                        var lastName = row[3] is DBNull ? "-" : (string)row[3];
                        var firstName = row[4] is DBNull ? "-" : (string)row[4];
                        var middleName = row[5] is DBNull ? "-" : (string)row[5];
                        var passSeries = row[6] is DBNull ? "-" : (string)row[6];
                        var passNo = row[7] is DBNull ? "-" : (string)row[7];
                        var disName = row[8] is DBNull ? "-" : (string)row[8];
                        var djamName = row[9] is DBNull ? "-" : (string)row[9];
                        var villName = row[10] is DBNull ? "-" : (string)row[10];
                        var street = row[11] is DBNull ? "" : (string)row[11];
                        var house = row[12] is DBNull ? "" : (string)row[12];
                        var flat = row[13] is DBNull ? "" : (string)row[13];
                        var regDate = row[14] is DBNull ? DateTime.MinValue : (DateTime)row[14];
                        var amount = row[15] is DBNull ? 0m : (decimal)row[15];

                        if (bankAccountId != Guid.Empty)
                        {
                            var reportItem = new ReportItem();
                            reportItem.AccountNo = accNo;
                            reportItem.ApplicationNo = appNo;                            
                            reportItem.LastName = lastName;
                            reportItem.FirstName = firstName;
                            reportItem.MiddleName = middleName;                            
                            reportItem.PassportSeries = passSeries;
                            reportItem.PassportNo = passNo;
                            if (!string.IsNullOrEmpty(disName))
                                reportItem.DistrictName = disName.ToString();
                            if (!string.IsNullOrEmpty(djamName))
                                reportItem.DjamoatName = djamName.ToString();
                            if (!string.IsNullOrEmpty(villName))
                                reportItem.VillageName = villName.ToString();
                            reportItem.Street = street;
                            reportItem.House = house;
                            reportItem.Flat = flat;
                            reportItem.RegDate = regDate;
        ...
        //Информация о получателях АСП
        public static class Report10v
        {
            public static readonly Guid UserId = new Guid("{E0F19306-AECE-477B-B110-3AD09323DD2D}");
            public static readonly string Username = "Admin";
            public static readonly Guid regionId = new Guid("{8c5e9217-59ac-4b4e-a41a-643fc34444e4}");
            ...
            public static List<RegionObject> GetAdministrativeDivision(List<ReportItem> reportItems, List<ReportItem> reportItemsOnPayment)
            {
                List<RegionObject> regionList = new List<RegionObject>();
                List<DjamoatObject> djamoatList = new List<DjamoatObject>();
                List<DistrictObject> districtList = new List<DistrictObject>();
                ...
                foreach (var djamoat in _djamoatList)
                {
                    ...
                    _djamoat.quantityAssignedFromBeginOfYear = reportItems.Where(x => x.DjamoatId.Equals(djamoat.Id) && (x.StateId.Equals(AssignedStateId) || x.StateId.Equals(OnPaymentStateId))).ToList().Count;
                    _djamoat.quantityOnPaymentFromBeginOfYear = reportItemsOnPayment.Where(x => x.DjamoatId.Equals(djamoat.Id) && x.StateId.Equals(OnPaymentStateId)).ToList().Count() + reportItems.Where(x => x.DjamoatId.Equals(djamoat.Id) && (x.StateId.Equals(AssignedStateId))).ToList().Count;
                ...
                foreach (var district in _districtList)
                {
                    ...
                    _district.quantityAssignedFromBeginOfYear = reportItems.Where(x => (x.DistrictId.Equals(district.Id)) && (x.StateId.Equals(AssignedStateId) || x.StateId.Equals(OnPaymentStateId))).Count();
                    _district.quantityOnPaymentFromBeginOfYear = reportItemsOnPayment.Where(x => (x.DistrictId.Equals(district.Id)) && (x.StateId.Equals(OnPaymentStateId))).Count() + reportItems.Where(x => (x.DistrictId.Equals(district.Id)) && (x.StateId.Equals(AssignedStateId))).ToList().Count;
                ...
                foreach (var region in areaList)
                {
                    regionList.Add
                                    (new RegionObject
                                    {
                                        ...
                                        quantityAssignedFromBeginOfYear = reportItems.Where(x => (x.RegionId.Equals(region.Id)) && (x.StateId.Equals(AssignedStateId) || x.StateId.Equals(OnPaymentStateId))).Count(),
                                        quantityOnPaymentFromBeginOfYear = reportItemsOnPayment.Where(x => (x.RegionId.Equals(region.Id)) && (x.StateId.Equals(OnPaymentStateId))).Count() + reportItems.Where(x => (x.RegionId.Equals(region.Id)) && (x.StateId.Equals(AssignedStateId))).ToList().Count,
            ...
            public static List<RegionObject> Execute()
            {
                var reportItems = GetReportItems();
                var reportItemsOnPayment = GetReportItemsOnPayment();
                var regionList = GetAdministrativeDivision(reportItems, reportItemsOnPayment);
                return regionList;
            }
            ...
            public static List<ReportItem> GetReportItemsOnPayment()
            {
                var context = CreateAsistContext("sod_user", new Guid("{4296EF4D-ED7A-41F8-BE88-2684AD21AC0E}"));
                var docRepo = context.Documents;
                var items = new List<ReportItem>();
                ...
                {
                    var appStateSource = query.JoinSource(query.Source, AppStateDefId, SqlSourceJoinType.Inner, "Application_State");
                    query.AndCondition(query.Source, "&State", ConditionOperation.Equal, OnPaymentStateId);
                    query.AddAttribute(appStateSource, "DistrictId");
                    query.AddAttribute(appStateSource, "DjamoatId");
                    query.AddAttribute(query.Source, "&State");
                    query.AddAttribute(appStateSource, "All");
        ...
        public static class GetStatisticDataInput
        {
            ...
            public static readonly Guid AppStateDefId = new Guid("{547BBA55-2281-4388-A1FC-EE890168AC2D}");
            //public static readonly Guid OrgInfoId = new Guid("{12DE71B2-6E65-4D5F-9781-795E4BB89F32}");
            public static readonly Guid OrgInfoId = new Guid("{A9DCB40D-369A-43E6-86D7-C33AA46C7525}");
            public static List<OrgInfo> GetOrgList(WorkflowContext context)
            {
                List<OrgInfo> orgInfos = new List<OrgInfo>();
                var docRepo = context.Documents;
                var docDefRepo = context.DocDefs;
                var docDef = docDefRepo.DocDefById(OrgInfoId);
                var qb = new QueryBuilder(OrgInfoId, context.UserId);
                var query = SqlQueryBuilder.Build(context.DataContext, qb.Def);
                var areaSrc = query.JoinSource(query.Source, regionId, SqlSourceJoinType.Inner, "Area");
                query.AddAttribute("OrgId");
                var regionAttr = query.AddAttribute(areaSrc, "&Id");
                using (var reader = new SqlQueryReader(context.DataContext, query))
                {
                    while (reader.Read())
                    {
                        var orgId = reader.IsDbNull(0) ? Guid.Empty : reader.GetGuid(0);
                        var areaId = reader.IsDbNull(1) ? Guid.Empty : reader.GetGuid(1);
                        orgInfos.Add(new OrgInfo { OrgId = orgId, RegionId = areaId });
                    }
                }
                return orgInfos;
            }
            ...
            // public static List<ReportItem> Execute(int year, int month)
            {
                var context = CreateAsistContext(Username, UserId);
                var r = GetOrgList(context);
            ...
            public static List<User> GetAdministrativeDivision(int year, int month)
            {
                ...
                var areaList = GetListFromSqlQueryReader(context, regionId, "Id1");
                var _districtList = GetListFromSqlQueryReader(context, districtId, "Area");
                var orgInfoList = GetOrgList(context);

                foreach (var district in orgInfoList)
                {
                    DistrictObject _district = new DistrictObject();
                    _district.Id = district.OrgId;
                    _district.Name = context.Orgs.GetOrgName(district.OrgId);
                    _district.RegionName = context.Documents.LoadById(district.RegionId)["Name"].ToString();

                    foreach (var subItems in reportItems.Where(x => x.OrgId.Equals(district.OrgId)).GroupBy(x => x.UserId))
                    {
                        int totalDoc = 0;
                        ...
                        {
                            user.QuantityDoc[i] = subItems.Where(x => (x.DocumentDate.Day == i + 1)).Count();
            ...
            public static List<ReportItem> GetReportItems(Guid reportId, int year, int month)
            {
                ...
                qb.Where("Date").Ge(fd).And("Date").Lt(ld).End();
                ...
                using (var query = context.CreateSqlQuery(qb.Def))
                {
                    ...
                    using (var reader = context.CreateSqlReader(query))
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            var userId = reader.IsDbNull(4) ? Guid.Empty : reader.GetGuid(4);
                            var orgId = reader.IsDbNull(5) ? Guid.Empty : reader.GetGuid(5);
                            if (districtId == Guid.Empty && regionId == Guid.Empty && docId == Guid.Empty) continue;
                            var item = new ReportItem { RegionId = regionId, DistrictId = districtId, UserId = userId, DocumentDate = docDate, OrgId = orgId };
        ...
        //СТАТИСТИЧЕСКИЕ ДАННЫЕ (10 отчетов)
        public static class StateReport_1
        {
            public static List<ReportItem> Execute()
            {
                /*var date = DateFormation;
                if (date == DateTime.MinValue)
                    throw new ApplicationException("Ошибка в периоде!");*/
                ...
                var items = new List<ReportItem>();
                var context = CreateAsistContext("sod_user", new Guid("{4296EF4D-ED7A-41F8-BE88-2684AD21AC0E}"));
                return CalcItems(context, items);
                //return items;
            }
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                var qb = new QueryBuilder(appDefId); //заявления
                qb.Where("&State").In(new object[] { assignedStateId, paydStateId }); //назначенные и на выплате
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    var appStateSrc = query.JoinSource(query.Source, appSatetDefId, SqlSourceJoinType.Inner, "Application_State");
                    var regionSrc = query.JoinSource(appStateSrc, regionDefId, SqlSourceJoinType.Inner, "RegionId");
                    var districtSrc = query.JoinSource(appStateSrc, districtDefId, SqlSourceJoinType.Inner, "DistrictId");

                    query.AddAttribute(query.Source, "&Id");
                    query.AddAttribute(regionSrc, "Name");
                    query.AddAttribute(districtSrc, "Name");
                    query.AddAttribute(districtSrc, "District_Type");
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var districtType = row[3] is DBNull ? Guid.Empty : (Guid)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    item.gr1 += districtName;

                                    if (districtType == villageTypeId)
                                    {
                                        item.gr2++;
                                    }
                                    if (districtType == cityTypeId)
                                    {
                                        item.gr3++;
                                    }
                                    item.gr4 = item.gr2 + item.gr3;
                                }
                            }               
                            items.Add(item);
                        }
                    ...
                    var items2 = new List<ReportItem>();
                    i = 1;
                    foreach (var subItems in items.GroupBy(x => x.gr1))
                    {
                        items2.Add(new ReportItem
                        {
                            gr1 = subItems.Key,
                            no = i,                            
                            gr2 = subItems.Sum(x => x.gr2),
                            gr3 = subItems.Sum(x => x.gr3),
                            gr4 = subItems.Sum(x => x.gr4)
                        });
                        i++;
                    }
                    return items2;
            ...
            private static readonly Guid regionDefId = new Guid("{8C5E9217-59AC-4B4E-A41A-643FC34444E4}"); //Область
            private static readonly Guid districtDefId = new Guid("{4D029337-C025-442E-8E93-AFD1852073AC}"); //Район
            private static readonly Guid cityTypeId = new Guid("{DE3D24A1-64D3-4647-B711-31F334D24416}"); //Тип Город
            private static readonly Guid villageTypeId = new Guid("65DF9C4D-A18B-4080-855F-4800E2A19C35"); //Тип Село
            ...
            public class ReportItem
            {
                public int no { get; set; } //порядковый номер
                public string gr1 { get; set; } //Наименование город/район
                public int gr2 { get; set; } //Тип "Село"
                public int gr3 { get; set; } //Тип "Город"
                public int gr4 { get; set; } //Всего ДМХ
            }
        }

        //Название индикатора: Род пола
        public static class StateReport_2
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var genderType = row[3] is DBNull ? Guid.Empty : (Guid)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    if (genderType == manTypeId)
                                    {
                                        item.gr2++;
                                    }
                                    if (genderType == womanTypeId)
                                    {
                                        item.gr3++;
                                    }
                                    item.gr4 = item.gr2 + item.gr3;
            ...
            private static readonly Guid regionDefId = new Guid("{8C5E9217-59AC-4B4E-A41A-643FC34444E4}"); //Область
            private static readonly Guid districtDefId = new Guid("{4D029337-C025-442E-8E93-AFD1852073AC}"); //Район
            private static readonly Guid manTypeId = new Guid("{74C6C7FE-53C6-4492-A62F-65A7A49AB644}"); //мужчины
            private static readonly Guid womanTypeId = new Guid("{56E07640-5B5B-47FA-832D-A6639F36EB71}"); //женщины
        ...
        //Название индикатора: сектор занятости
        public static class StateReport_3
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    query.AddAttribute(regionSrc, "Name");
                    query.AddAttribute(districtSrc, "Name");
                    query.AddAttribute(appStateSrc, "employment");
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var employmentType = row[3] is DBNull ? Guid.Empty : (Guid)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    item.gr0 += districtName;

                                    if (employmentType == employmentType1)
                                    {
                                        item.gr1++;
                                    }
                                    if (employmentType == employmentType2)
                                    {
                                        item.gr2++;
                                    }
                                    if (employmentType == employmentType3)
                                    {
                                        item.gr3++;
                                    }
                                    if (employmentType == employmentType4)
                                    {
                                        item.gr4++;
                                    }
                                    if (employmentType == employmentType5)
                                    {
                                        item.gr5++;
                                    }
                                    if (employmentType == employmentType6)
                                    {
                                        item.gr6++;
                                    }
                                    if (employmentType == employmentType7)
                                    {
                                        item.gr7++;
                                    }
                                    if (employmentType == employmentType8)
                                    {
                                        item.gr8++;
                                    }
                                    if (employmentType == employmentType9)
                                    {
                                        item.gr9++;
                                    }
                                    if (employmentType == employmentType10)
                                    {
                                        item.gr10++;
                                    }
                                    if (employmentType == employmentType11)
                                    {
                                        item.gr11++;
                                    }
                                    if (employmentType == employmentType12)
                                    {
                                        item.gr12++;
                                    }
                                    if (employmentType == employmentType13)
                                    {
                                        item.gr13++;
                                    }
                                    if (employmentType == employmentType14)
                                    {
                                        item.gr14++;
                                    }
                                    if (employmentType == employmentType15)
                                    {
                                        item.gr15++;
                                    }
                                    if (employmentType == employmentType16)
                                    {
                                        item.gr16++;
                                    }
                                    if (employmentType == employmentType17)
                                    {
                                        item.gr17++;
                                    }
                                    if (employmentType == employmentType18)
                                    {
                                        item.gr18++;
                                    }
                                    item.gr19 = item.gr1 + item.gr2 + item.gr3 + item.gr4 + item.gr5 + item.gr6 + item.gr7 + item.gr8 + item.gr9 + item.gr10 + item.gr11 + item.gr12 + item.gr13 + item.gr14 + item.gr15 + item.gr16 + item.gr17 + item.gr18;
                                }
                            }
                            items.Add(item);
                        }
                    ...
                    var items2 = new List<ReportItem>();
                    i = 1;
                    foreach (var subItems in items.GroupBy(x => x.gr0))
                    {
                        items2.Add(new ReportItem
                        {
                            gr0 = subItems.Key,
                            no = i,
                            gr1 = subItems.Sum(x => x.gr1),
                            gr2 = subItems.Sum(x => x.gr2),
                            gr3 = subItems.Sum(x => x.gr3),
                            gr4 = subItems.Sum(x => x.gr4),
                            gr5 = subItems.Sum(x => x.gr5),
                            gr6 = subItems.Sum(x => x.gr6),
                            gr7 = subItems.Sum(x => x.gr7),
                            gr8 = subItems.Sum(x => x.gr8),
                            gr9 = subItems.Sum(x => x.gr9),
                            gr10 = subItems.Sum(x => x.gr10),
                            gr11 = subItems.Sum(x => x.gr11),
                            gr12 = subItems.Sum(x => x.gr12),
                            gr13 = subItems.Sum(x => x.gr13),
                            gr14 = subItems.Sum(x => x.gr14),
                            gr15 = subItems.Sum(x => x.gr15),
                            gr16 = subItems.Sum(x => x.gr16),
                            gr17 = subItems.Sum(x => x.gr17),
                            gr18 = subItems.Sum(x => x.gr18),
                            gr19 = subItems.Sum(x => x.gr19)
                        });
                        i++;
                    }
                    return items2;
            ...
            private static readonly Guid regionDefId = new Guid("{8C5E9217-59AC-4B4E-A41A-643FC34444E4}"); //Область
            private static readonly Guid districtDefId = new Guid("{4D029337-C025-442E-8E93-AFD1852073AC}"); //Район
            private static readonly Guid employmentType1 = new Guid("{0B8F9451-A7E6-4BE5-8A9D-5F4B87B9BEC4}");
            private static readonly Guid employmentType2 = new Guid("{7B88FA9C-046A-4410-8B31-02AF9D3CDF3B}");
            private static readonly Guid employmentType3 = new Guid("{812FD762-A1A4-4D0E-A8BA-02FCC83983DA}");
            private static readonly Guid employmentType4 = new Guid("{DF4D8FD3-9787-4B72-ABB7-04049C698C90}");
            private static readonly Guid employmentType5 = new Guid("{67EEC1CE-A9AF-45E0-A1ED-0D5D77B8C89F}");
            private static readonly Guid employmentType6 = new Guid("{D6C76C4E-3FCC-4DFE-8ECA-33EA5A99C100}");
            private static readonly Guid employmentType7 = new Guid("{04E2F558-9AB5-4174-939B-34705A0E4637}");
            private static readonly Guid employmentType8 = new Guid("{909A320F-62E6-42D7-AD12-5343D450C07D}");
            private static readonly Guid employmentType9 = new Guid("{54FCA68E-4D46-4073-A07E-584D224D8D5C}");
            private static readonly Guid employmentType10 = new Guid("{1B9A49F3-AEF4-4CAE-A550-5BE8719DC48C}");
            private static readonly Guid employmentType11 = new Guid("{9AA353B1-A900-4576-A00E-7C73F2CB5C10}");
            private static readonly Guid employmentType12 = new Guid("{17470B71-305A-425A-9261-9D70D82EFFE5}");
            private static readonly Guid employmentType13 = new Guid("{8914ED1E-277C-497D-B0B9-9E26DDDAED95}");
            private static readonly Guid employmentType14 = new Guid("{D4286218-2B2B-42F6-8EAC-AD0296EA3585}");
            private static readonly Guid employmentType15 = new Guid("{7FC56262-6947-4FDA-ABE5-BEC95367B0AD}");
            private static readonly Guid employmentType16 = new Guid("{199E785E-23FB-4544-B86F-F1ADF68ED518}");
            private static readonly Guid employmentType17 = new Guid("{3B9655AD-B9FD-4EE8-BD06-277AB45549F2}");
            private static readonly Guid employmentType18 = new Guid("{8A7C4B40-E412-4AC6-B6AD-AC90A982DF1F}");
            ...
            {
                public int no { get; set; } //порядковый номер
                public string gr0 { get; set; } //Наименование город/район
        ...
        //Название индикатора: количество членов дмх (бенефициары)
        public static class StateReport_4
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var allFamCount = row[3] is DBNull ? 0 : (int)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    {
                                        item.gr15++;
                                        item.gr15_ += allFamCount; //итого бенефициаров гр.15
                                    }                                    
                                    item.gr16 = item.gr1 + item.gr2 + item.gr3 + item.gr4 + item.gr5 + item.gr6 + item.gr7 + item.gr8 + item.gr9 + item.gr10 + item.gr11 + item.gr12 + item.gr13 + item.gr14 + item.gr15;
                                    item.gr17 += allFamCount;           
                                }
                            }
                            items.Add(item);
                        }
            ...
            public class ReportItem
            {
                ...
                public int gr14 { get; set; }
                public int gr15 { get; set; }
                public int gr15_ { get; set; }
                public int gr16 { get; set; }
                public int gr17 { get; set; }
            }
        }
        //Название индикатора: количество членов дмх (дети до 16 лет)
        public static class StateReport_5
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    query.AddAttribute(regionSrc, "Name");
                    query.AddAttribute(districtSrc, "Name");
                    query.AddAttribute(appStateSrc, "Children_under_16");
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var child16Count = row[3] is DBNull ? 0 : (int)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    {
                                        item.gr10++;
                                        item.gr10_ += child16Count; //итого детей до 16 лет гр.10
                                    }
                                    item.gr11 = item.gr1 + item.gr2 + item.gr3 + item.gr4 + item.gr5 + item.gr6 + item.gr7 + item.gr8 + item.gr9 + item.gr10;
                                    item.gr12 += child16Count;
                                }
                            }
                            items.Add(item);
                        }
            ...
            public class ReportItem
            {
                ...
                public int gr9 { get; set; }
                public int gr10 { get; set; }
                public int gr10_ { get; set; }
                public int gr11 { get; set; }
                public int gr12 { get; set; }
            }
        }
        //Название индикатора: количество инвалидов 1 гр. в составе ДМХ
        public static class StateReport_6
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    query.AddAttribute(regionSrc, "Name");
                    query.AddAttribute(districtSrc, "Name");
                    query.AddAttribute(appStateSrc, "group_1");
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var firstGroup = row[3] is DBNull ? 0 : (int)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    item.gr6 = item.gr1 + item.gr2 + item.gr3 + item.gr4 + item.gr5;
                                    item.gr7 += firstGroup;
                                }
                            }
                            items.Add(item);
        ...
        //Название индикатора: Получатели - инвалиды 2 группы
        public static class StateReport_7
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    var regionSrc = query.JoinSource(appStateSrc, regionDefId, SqlSourceJoinType.Inner, "RegionId");
                    var districtSrc = query.JoinSource(appStateSrc, districtDefId, SqlSourceJoinType.Inner, "DistrictId");
                    var famMemSrc = query.JoinSource(appStateSrc, famDefId, SqlSourceJoinType.Inner, "Family_Member");

                    query.AddAttribute(query.Source, "&Id");
                    query.AddAttribute(regionSrc, "Name");
                    query.AddAttribute(districtSrc, "Name");
                    query.AddAttribute(appStateSrc, "DisabilityGroupe"); //гр. инвалидности главы ДМХ
                      query.AddAttribute(famMemSrc, "DisablilityGroupe"); //гр. инвалидности ЧДМХ
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var secondGroup = row[3] is DBNull ? Guid.Empty : (Guid)row[3];
                        var famMemSecondGroup = row[4] is DBNull ? Guid.Empty : (Guid)row[4];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    if (secondGroup == IIgroup)
                                    {
                                        item.gr1++;
                                    }
                                    if (famMemSecondGroup == IIgroup)
                                    {
                                        item.gr2++;
                                    }
                                    item.gr3 = item.gr1 + item.gr2;
                                }
                            }
                            items.Add(item);
                        }
            ...
            private static readonly Guid regionDefId = new Guid("{8C5E9217-59AC-4B4E-A41A-643FC34444E4}"); //Область
            private static readonly Guid districtDefId = new Guid("{4D029337-C025-442E-8E93-AFD1852073AC}"); //Район
            private static readonly Guid famDefId = new Guid("{959762D8-FEB6-4247-986D-7ECE63EED1AD}"); //Члены ДМХ

            private static readonly Guid Igroup = new Guid("{CC195B10-995A-44DB-A852-F519E0AD1BED}");
            private static readonly Guid IIgroup = new Guid("{5FB9A008-071E-4FD5-B027-E197B4C833E0}");
            private static readonly Guid IIIgroup = new Guid("{715D3EF3-A08B-47C6-8C5E-2555A5E1253A}");
        ...
        //Название индикатора: Получатели - инвалиды 3 группы
        public static class StateReport_8
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    if (secondGroup == IIIgroup)
                                    {
                                        item.gr1++;
                                    }
                                    if (famMemSecondGroup == IIIgroup)
                                    {
                                        item.gr2++;
                                    }
                                    item.gr3 = item.gr1 + item.gr2;
        ...
        //Название индикатора: Дети инвалиды в составе ДМХ
        public static class StateReport_9
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    query.AddAttribute(regionSrc, "Name");
                    query.AddAttribute(districtSrc, "Name");
                    query.AddAttribute(appStateSrc, "children_disabilities");
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var childDis = row[3] is DBNull ? 0 : (int)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    item.gr4 = item.gr1 + item.gr2 + item.gr3;
                                    item.gr5 += childDis;
                                }
                            }
                            items.Add(item);
        ...
        //Название индикатора: Престарелые граждане старше 65 лет составе ДМХ
        public static class StateReport_10
        {
            public static List<ReportItem> Execute()
            {
                var items = new List<ReportItem>();
            ...
            public static List<ReportItem> CalcItems(WorkflowContext context, List<ReportItem> items)
            {
                ...
                using (var query = SqlQueryBuilder.Build(context.DataContext, qb.Def))
                {
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        var regionName = row[1] is DBNull ? "" : row[1].ToString();
                        var districtName = row[2] is DBNull ? "" : row[2].ToString();
                        var old65 = row[3] is DBNull ? 0 : (int)row[3];
                        ...
                        if (appId != Guid.Empty)
                        {
                            if (!string.IsNullOrEmpty(regionName))
                            {
                                if (!string.IsNullOrEmpty(districtName))
                                {
                                    ...
                                    item.gr5 = item.gr1 + item.gr2 + item.gr3 + item.gr4;
                                    item.gr6 += old65;
                                }
                            }
                            items.Add(item);
